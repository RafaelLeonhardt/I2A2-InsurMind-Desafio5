...
- **Completed**: `spec.md`+`design.md`+`tasks.md` das 25 histórias (2.1–5.9) — ver auditoria de consistência cruzada e ADs no histórico desta seção. **História 2.1 (Coletar e normalizar dados do INMET) implementada e verificada PASS** (2026-09-01/02): 14 tasks (T1–T14) commitadas `43e3e3b..44fd7af`; Verifier rodada 1 encontrou FAIL (1 mutante sobrevivente no intervalo de coleta automática + 4 lacunas de cobertura ancorada em AC — INMET-03/04/06/07/15); Fix 1–4 commitados em `552b378` (inclui correção real de spec-precision na UI: estados de sincronização exibiam o enum cru em vez dos rótulos `Coletando`/`Normalizando`/`Concluído`/`Falha` exigidos pelo spec); Verifier rodada 2 (independente, `c95279f`) confirmou PASS — 15/17 ACs plenamente ancorados, sensor 2/2 morto, gate verde. `validate_state.py` confirma 0 erros. Fix 5 (prova ao vivo do INMET real, T4) permanece dívida não-bloqueante documentada em `adaptadores/meteorologia/README.md:17-34` — sem rede disponível na sessão de implementação.
- **In-progress**: nada — História 2.1 fechada (implementação + fix + verificação, todos commitados; `git status` limpo).
- **Next step**: implementar **História 2.2** (Operar com segurança durante indisponibilidades meteorológicas), seguindo a mesma ordem de dependência já registrada: Épico 2 (2.1 ✅ →2.2→2.3/2.4→2.5→2.6) → Épico 3 (3.1→3.2→3.3→3.4→3.5→3.6) → Épico 4 (4.1/4.3 em paralelo lógico, 4.2 depois de 3.5/3.6, 4.4 por último) → Épico 5 (5.1–5.6 independentes/paralelizáveis, 5.7 depende de 5.1–5.6, 5.8 depende de tudo, 5.9 por último, publicação final sempre sob autorização explícita do usuário). Cada história segue o mesmo protocolo desta sessão: implementar as tasks → gate verde → Verifier automático → fix→re-verify (máx. 3 rodadas) → commit da verificação → atualizar este Handoff.
- **Review**: revisão independente do planejamento (2026-08-30) produziu 8 achados. A1–A4 (críticos/altos) corrigidos como AD-012–AD-015. **A5–A8 (médios) permanecem abertos**, a resolver no fluxo normal das histórias: A5 costura agendador↔runner sem decisão (coleta agendada não cria execução; RUNNER-01 insatisfeito para coletas automáticas — relevante a partir da 2.2), A6 contradição spec↔design na 2.6 sobre o terminal técnico, A7 "medidas normalizadas" ausentes do modelo `EventoMeteorologico` + critério de atribuição de `tipo` não especificado (A7 ficou parcialmente mitigado pela Fix 4 da 2.1, que ancorou os campos temporais — mas o critério de atribuição de `tipo` em si segue sem decisão formal), A8 chave de dedup de eventos sem `proveniencia` (evento sintético pode suprimir evento real idêntico — relevante a partir da 2.2, cenário sintético de contingência).
- **Blockers**: nenhum. Riscos conhecidos: (1) Fix 5 da 2.1 (prova ao vivo do INMET) segue pendente por falta de rede na sessão — não bloqueia 2.2 em diante; (2) `langchain`/`langchain-openai`/`langgraph` (3.1) e Playwright/`@axe-core/playwright` (5.8) são dependências novas sem versão fixada; (3) agregação de 4.4 lê ~9 fontes (custo aceito de PoC); (4) ferramenta de conversão Markdown→PDF (5.9) ainda não escolhida; (5) achados A5–A8 acima; (6) `EstadoSincronizacao.NORMALIZANDO` (2.1) é declarado no enum e no `CHECK` da migração mas nunca persistido por nenhum caminho de código real — observação não-bloqueante da Verificação rodada 2, vale relembrar se uma história futura tentar usar esse estado.
