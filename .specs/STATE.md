...
- **Completed**: Histórias 5.1–5.8 implementadas e verificadas **PASS** (ver Handoffs anteriores e `.specs/features/*/validation.md` de cada uma). **História 5.9 (Preparar e empacotar a entrega final) — T1–T4 implementadas e verificadas PASS em 2026-09-07**; T5 (publicação) permanece pendente por decisão explícita do usuário (ver abaixo). T1–T4: três scripts de orquestração pura em `scripts/` (`gerar_relatorio_tecnico.py`, `empacotar_entrega.py`, `gerar_inventario.py`), governados por um `pyproject.toml`/`uv.lock` **novos na raiz do repositório** (rodar com `uv run --directory . pytest testes/`) — projeto Python independente de `src/backend`, sem conflito de workspace. `gerar_relatorio_tecnico.py` monta `docs/entrega/relatorio-tecnico.pdf` via `fpdf2` (única opção pura-Python disponível no ambiente; pandoc/wkhtmltopdf/weasyprint não instalados) a partir de README/ADRs/docs/design/`.specs/STATE.md`/`docs/evidencias/` (5.8) e da constante de mensagens sanitizadas de `testes-e2e/suporte/servidor-dubles.ts`; falha explicitamente (`EvidenciaAusente`) se qualquer artefato citado faltar. `empacotar_entrega.py` monta `docs/entrega/entrega.zip` a partir de `git ls-files` (já respeita `.gitignore`) com um filtro defensivo redundante. `gerar_inventario.py` grava SHA-256/tamanho de cada artefato em `docs/entrega/inventario.json`. `docs/entrega/` é gitignorado (artefatos regeneráveis, mesmo espírito de `var/`) — mas a evidência de validação de checkout/ZIP (T4) foi movida para `docs/evidencias/validacao-checkout-entrega.md`, rastreada pelo Git e incluída no ZIP, porque essa evidência específica precisa acompanhar a entrega publicada (correção pós-Verifier, ver `design.md` Tech Decisions). Verifier: **rodada 1 FAIL** (gate verde, mas sensor só matou 2/7 mutantes — asserções comparavam o PDF contra a própria função extratora em vez de marcadores lidos direto da fonte; filtro defensivo do ZIP sem teste direto; bug real encontrado em `_esta_excluido()` que só casava padrões como `.venv/`/`__pycache__/` na raiz, nunca em subdiretório; ENTREGA-03 sem task dona) → 4 commits de correção (`0b2a126`, `1c56ae9`, `be3c61a`, `5319eac`) → **rodada 2 PASS** — 11/12 mutantes mortos, 70 testes verdes, ENTREGA-01..05 marcadas **Verified** em `spec.md`, 1 spec-precision gap (M10) fechado no mesmo commit de fechamento (`4a855fb`). Ver `.specs/features/5-9-.../validation.md` para o relatório completo de ambas as rodadas.
- **Next step**: **T5 (publicação autorizada) é a única peça pendente do Épico 5 inteiro.** O usuário foi consultado nesta sessão ("Publicar agora?") e respondeu **"Not now"** — decisão explícita de não autorizar a publicação neste momento, um estado terminal válido e previsto pela própria AC ENTREGA-06 (a história continua pendente até autorização explícita separada). Quando o usuário pedir para publicar: (1) confirmar destino (serviço, repositório novo ou existente, visibilidade); (2) publicar manualmente via `git`/`gh` (nunca script automatizado, por design); (3) em caso de sucesso, marcar ENTREGA-06/07 como Verified e a História 5.9 (e o Épico 5 inteiro) como concluída; (4) em caso de falha, registrar o erro sanitizado em `docs/evidencias/` ou similar, preservar `docs/entrega/` intacto, e manter a história pendente para nova tentativa. Não presumir nem executar a publicação sem esse pedido explícito.
