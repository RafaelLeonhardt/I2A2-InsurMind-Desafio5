...
- **Next step**: Continuar o Épico 6 pela História 6.2 (Acompanhar a execução e a decisão do evento — já tem `spec.md`, falta Design/Tasks/Execute), que depende diretamente de 6.1 (usa a lista de eventos e o mecanismo AD-016 já prontos). Depois, na ordem de dependência: 6.3, 6.4, 6.7 (independentes entre si, dependem só de 6.1), depois 6.5 → 6.6, e por fim 6.8/6.9 (lado Segurado, independentes do restante do épico).
- **Known open items (not blocking anything)**:
  ...
  - Histórias 6.2–6.9 têm `spec.md` mas ainda não passaram por Design/Tasks/Execute — 6.5 e 6.6 devem seguir o padrão AD-016 (`Superficie` com payload) já estabelecido em 6.1, em vez de reabrir a discussão de arquitetura de navegação.
