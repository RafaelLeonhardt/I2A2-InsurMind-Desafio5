...
- **Completed**: Histórias 5.1–5.7 implementadas e verificadas **PASS**. História 5.7 (Alternar o segurado sintético com contexto consistente) fechada em 2026-09-06/07: T1–T5 `b790553..9ab6c0f` (backend `listar_sinteticos`+`GET /segurados`; `SeguradoContexto` provider React; `SeletorSegurado` "Visualizar como"; `PainelSegurado` compondo as 5 superfícies de 5.1–5.6 — SPEC_DEVIATION: novo componente de composição em vez de editar as 5 superfícies existentes, ver `design.md`/`tasks.md`). Verifier rodada 1 (`594ecf0`) achou FAIL: único gap Blocker — `PainelSegurado` correto mas não alcançável em `App.tsx`. Fix 1 (`a57df76`) montou `PainelSegurado` no caso `'visao-geral'` do perfil Segurado; Verifier rodada 2 (`7edec03`) **PASS pleno** — 8/8 SELETOR-01..08 verificados, sensor 4/4 mutante morto, 1095 backend + 441 frontend testes verdes. Detalhe completo de cada história 5.1–5.7: ver `git log` e `.specs/features/*/validation.md` de cada uma — este Handoff não repete histórico já registrado ali.
- **Next step**: Próxima história: **5.8 (Executar e comprovar os cenários ponta a ponta)** — spec/design/tasks já existem em `.specs/features/5-8-executar-e-comprovar-os-cenarios-ponta-a-ponta/`. Mesmo protocolo: worker de implementação (todas as tasks, um commit por task) → gate verde → Verifier independente automático → fix→re-verify (máx. 3 rodadas) → commit da verificação → atualizar este Handoff → próxima história (5.9).
- **Known open items (not blocking 5.8)**:
  - Superfícies órfãs do perfil **Administrador** dos Épicos 3/4 (revisão de lote, resultados, detalhe de resultado, linha do tempo, explicação de comunicado, etc.) ainda não ligadas a `App.tsx`/`PerfilContexto.tsx` — 5.7 resolveu isso apenas para as 5 superfícies do perfil **Segurado** (agora via `PainelSegurado`). Considerar história dedicada de integração de UI para o lado Administrador se o produto precisar demonstrá-las.
...
- **Lições consolidadas ativas** (aplicar em toda história nova, ver `.specs/LESSONS.md` para a lista completa confirmada/candidata): variar valores de campos irmãos na mesma fixture desde a primeira escrita em toda camada nova; todo módulo `api/*.ts` precisa de teste próprio para a tradução `snake_case`→`camelCase`; testar o valor não-padrão de todo campo booleano/enum em cada camada; atualizar `test_openapi_sincronizado.py` e `test_saude.py` juntos ao adicionar rota; atualizar a tabela de rastreabilidade de `spec.md` a cada task concluída; declarar SPEC_DEVIATION explícito com razão sempre que a implementação divergir do design; montar toda região `aria-live` sempre presente e vazia, nunca já preenchida; **nova desde 5.7 (L-079/L-080, candidatas)**: antes de fechar uma história de UI, confirmar que o componente novo está de fato montado e alcançável a partir de `App.tsx`/navegação real — um componente corretamente testado mas nunca montado entrega zero valor ao usuário; um SPEC_DEVIATION que troca "editar 5 arquivos" por "um novo componente de composição" deve também declarar onde esse componente é conectado à aplicação real, não só justificar a escolha estrutural.
