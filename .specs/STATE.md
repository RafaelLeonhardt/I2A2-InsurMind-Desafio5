...
- **Completed**: `spec.md`+`design.md`+`tasks.md` das 25 histórias (2.1–5.9) — ver auditoria de consistência cruzada e ADs no histórico desta seção. **História 2.1 (Coletar e normalizar dados do INMET) implementada e verificada PASS** (2026-09-01/02): ver detalhe na entrada anterior desta seção (Fix 1–4 em `552b378`, PASS confirmado em `c95279f`). **História 2.2 (Operar com segurança durante indisponibilidades meteorológicas) implementada e verificada PASS** (2026-09-02): 7 tasks (T1–T7) commitadas `e7a3410..2a6efa8` — `ColetorComRetry` (3 tentativas, backoff 1s/2s), `RepositorioExecucaoPreventiva`/`RepositorioExcecoesOperacionais` (primeira escrita real em `execucao_preventiva`, AD-008), `AdaptadorCenarioSintetico` (relógio corrente em vez de valores históricos literais, evita colidir com a `UNIQUE` de T1), `solicitar_nova_tentativa`/`ativar_cenario_sintetico`, e os 6 estados da fonte no frontend (`calcularEstadoFonte`, precedência documentada no código). Verifier rodada 1 (`a645af53...`) encontrou FAIL: 1 mutante sobrevivente (ordem de prioridade `recuperada`/`degradada`) + 2 gaps de comportamento (RESIL-06 "idade calculada" nunca implementada, RESIL-09 snapshot não marcado como desatualizado) + 4 lacunas de precisão; Fix 1–6 commitados em `50e31fa`. Rodada 2 confirmou os Fix 1–6 genuínos mas achou 2 lacunas residuais só de teste (limites exatos de `calcularIdade`, `excecoes_operacionais` nunca lida de volta do DuckDB real); Fix 7–8 commitados em `5575981`. Rodada 3 (`67ec913`) confirmou **PASS pleno — 18/18 RESIL verificados**, sensor 2/2 morto, zero regressão (`git diff 50e31fa..5575981 -- src/` = +38/−0, nenhuma linha de produção). `validate_state.py` confirma 0 erros para ambas as histórias.
- **In-progress**: nada — Histórias 2.1 e 2.2 fechadas (implementação + fix + verificação, todos commitados; `git status` limpo).
- **Next step**: implementar **História 2.3** (Identificar eventos meteorológicos relevantes), seguindo a mesma ordem de dependência já registrada: Épico 2 (2.1 ✅ → 2.2 ✅ → 2.3 → 2.4 → 2.5 → 2.6) → Épico 3 (3.1→3.2→3.3→3.4→3.5→3.6) → Épico 4 (4.1/4.3 em paralelo lógico, 4.2 depois de 3.5/3.6, 4.4 por último) → Épico 5 (5.1–5.6 independentes/paralelizáveis, 5.7 depende de 5.1–5.6, 5.8 depende de tudo, 5.9 por último, publicação final sempre sob autorização explícita do usuário). Cada história segue o mesmo protocolo já rodado duas vezes nesta sessão: implementar as tasks → gate verde → Verifier automático → fix→re-verify (máx. 3 rodadas) → commit da verificação → atualizar este Handoff.
