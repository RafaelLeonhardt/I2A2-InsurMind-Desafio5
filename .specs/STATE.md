...
- **Completed**: `spec.md`+`design.md`+`tasks.md` das 25 histórias (2.1–5.9) — ver auditoria de consistência cruzada e ADs no histórico desta seção. **História 2.1 (Coletar e normalizar dados do INMET) implementada e verificada PASS** (2026-09-01/02): Fix 1–4 em `552b378`, PASS confirmado em `c95279f`. **História 2.2 (Operar com segurança durante indisponibilidades meteorológicas) implementada e verificada PASS** (2026-09-02): T1–T7 `e7a3410..2a6efa8`; Verifier rodada 1 (FAIL) → Fix 1–6 `50e31fa` → rodada 2 (2 lacunas residuais de teste) → Fix 7–8 `5575981` → rodada 3 (`67ec913`) **PASS pleno — 18/18 RESIL verificados**, sensor 2/2 morto. **História 2.3 (Identificar eventos meteorológicos relevantes) implementada e verificada PASS** (2026-09-02): T1–T6 commitadas `6b2108c..22b2906` — migração `0004_avaliacao_risco` (T1), `dominio/avaliador_risco.py` como função pura determinística (T2, AD-5: `test_camadas.py` proíbe `dominio/*` importar `adaptadores` via AST), `RepositorioRegras`/`RepositorioAvaliacoesRisco` com `regra_versao` como snapshot de valor não-referência (T3, AD-11), `ServicoAvaliacaoRisco` (T4), endpoint HTTP `GET /execucoes/{id}/avaliacao-risco` (T5), superfície "Evento e decisão" com 3 categorias por texto/ícone/cor (T6). Verifier rodada 1 (`22b2906`) achou FAIL: RISCO-09 não persistia o terminal `sem_risco` quando não havia regra ativa + mutante M2 sobrevivente (`regra_versao` fixture trivial) + 3 lacunas menores (RISCO-02 exemplos limítrofes só em teste, RISCO-12 distinção de ícone não asserida, constante morta); Fix 1–5 commitados em `1122288` — migração `0005_avaliacao_risco_sem_regra` relaxa `regra_id`/`regra_versao` para `NULL` via recreate-and-copy (AD-015) para permitir persistir o snapshot sem regra ativa. Rodada 2 confirmou **PASS pleno — 13/13 RISCO cobertos** (2 spec-precision gaps herdados e aceitos, ver Blockers), sensor 3/3 morto, 292 backend + 164 frontend testes verdes, todos os gates (`pytest`/`ruff`/`pyright`/`vitest`/`lint`/`build`) reexecutados pelo próprio Verificador. Achou 1 regressão de documentação não bloqueante (README ainda descrevia `regra_id`/`regra_versao` como `NOT NULL` após o Fix 1) e 1 lacuna cosmética de teste de frontend (estado `sem_regra_ativa` não exercitado na superfície) — o Verifier julgou que nenhuma justifica uma 3ª rodada; a doc foi corrigida e commitada em `20a2b98`, a lacuna cosmética fica registrada como não-bloqueante. `validate_state.py` confirma 0 erros para as três histórias.
- **In-progress**: nada — Histórias 2.1, 2.2 e 2.3 fechadas (implementação + fix + verificação, todos commitados; `git status` limpo).
- **Next step**: implementar **História 2.4** (Configurar, testar e versionar regras preventivas), seguindo a mesma ordem de dependência já registrada: Épico 2 (2.1 ✅ → 2.2 ✅ → 2.3 ✅ → 2.4 → 2.5 → 2.6) → Épico 3 (3.1→3.2→3.3→3.4→3.5→3.6) → Épico 4 (4.1/4.3 em paralelo lógico, 4.2 depois de 3.5/3.6, 4.4 por último) → Épico 5 (5.1–5.6 independentes/paralelizáveis, 5.7 depende de 5.1–5.6, 5.8 depende de tudo, 5.9 por último, publicação final sempre sob autorização explícita do usuário). Cada história segue o mesmo protocolo já rodado três vezes nesta sessão: implementar as tasks → gate verde → Verifier automático → fix→re-verify (máx. 3 rodadas) → commit da verificação → atualizar este Handoff.
