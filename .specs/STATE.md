...
- **Completed**: `spec.md`+`design.md`+`tasks.md` das 25 histórias (2.1–5.9), todas validadas por `validate_spec.py`/`validate_tasks.py` (0 erros; avisos de granularidade aceitos, mesma prática de `1-2-inicializar-e-restaurar-dados-sinteticos/tasks.md`). AD-006 (runner `asyncio` in-process) e AD-007 (mapeamento estação real→área sintética) registrados acima, do Épico 2 — nenhum outro épico introduziu AD novo; toda decisão de arquitetura adicional (LangGraph por mensagem em 3.2–3.5, `RetryComBackoff[T]` genérico em 3.1, execução correlacionada 3x em 2.2/3.1/3.6, agregação de leitura pura em 4.1–4.4/5.1–5.5, `SeguradoContexto` simétrico a `PerfilContexto` em 5.7, Playwright+`@axe-core/playwright` como par padrão de mercado em 5.8) decorre de ADRs/AD já aprovados ou de pesquisa registrada localmente em cada `design.md`.
